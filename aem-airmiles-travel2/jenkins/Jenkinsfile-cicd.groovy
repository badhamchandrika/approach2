...
//testing pipeline with valid commit message format

//small jenkins parameter validation check
if (JENKINS_PIPELINE_ID != 'staging' && JENKINS_PIPELINE_ID != 'hotfix_ci' && JENKINS_PIPELINE_ID != 'hotfix_staging') error("JENKINS_PIPELINE_ID string parameter in jenkins needs to be either 'code_quality', 'hotfix_code_quality', 'hotfix_staging', 'dev', 'qa' or 'staging'")

//testing staging pipeline

// GLOBAL STATIC Variables
EMAIL_RECIPIENTS = 'uyarra@loyalty.com,pkothapalli@loyalty.com,fcervantes@loyalty.com'
L1_REPO_NAME = 'aem-airmiles-travel2'
MICROSOFT_TEAMS_PROJECT_WEBHOOK = 'https://loyaltyone.webhook.office.com/webhookb2/9ca9d8d1-162f-4c3a-a2fc-23fa187c6bad@a9f9458b-a469-48c4-9a71-806e262869bf/JenkinsCI/bc9a243c3e334c14a13ab63e359894c8/279ce5db-ce0a-41c8-af9a-4585ceae6c94'

L1_REPO_TRUNK='main'
ADOBE_REPO_TRUNK='main'
ADOBE_REPO_ID=107004
...
def JENKINS_PIPELINES = [
    //ci:        [adobePipelineId: '10627957',  mutex: "${L1_REPO_NAME}-ci",          deployStageName: 'Adobe Code Quality Scan'],
    hotfix_ci: [adobePipelineId: '10627958',  mutex: "${L1_REPO_NAME}-hotfix-ci",   deployStageName: 'Hotfix Adobe Code Quality Scan'],
    ...
    hotfix_staging:      [adobePipelineId: '58806',  mutex: "ams-prod", deployStageName: 'Hotfix Staging Deployment']
    //staging:             [adobePipelineId: '7479542',    mutex: "ams-prod",        deployStageName: 'Adobe Staging Deployment'],
    //hotfix_staging:      [adobePipelineId: '7479542',  mutex: "ams-prod", deployStageName: 'Hotfix Staging Deployment']
]
            ...
            steps {
                ...
                script {
                    ...
                    lock(resource: mutex , variable: 'mutex') {
                       ...
                       /* if (JENKINS_PIPELINE_ID != 'ci' && JENKINS_PIPELINE_ID != 'hotfix_ci') {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'adobe-github-credentials', usernameVariable: 'adobe_git_user', passwordVariable: 'adobe_git_password')]) {
                            jenkinsUtils.maskExecute([adobe_git_password]) {
                                sshagent(credentials: ['jenkins-ssh-key'], ignoreMissing: false) {
                                    ...
                                    sh """
                                        git config --global --add safe.directory ${WORKSPACE}
                                        echo "Syncing Adobe Tags to main"
                                        ...
                                        elif [[ "${JENKINS_PIPELINE_ID}" == "staging" && "${status}" == "FINISHED" ]]; then
                                            echo "Pipeline status: $status"
                                            git pull adobeSync ${ADOBE_REPO_TRUNK} --tags
                                            git push origin ${L1_REPO_TRUNK} --tags 
                                        else
                                        echo "Error: Adobe Custom Tag Backup To L1 Github Failed."
                                        fi
                                    """
                                }
                            }
        ...
        script {
            ...
            /* if (JENKINS_PIPELINE_ID == 'staging' && !isJenkinsCommit()) {
                ...
                notifyParties(String.format(
                    ...
                    automationStatus, 
                    env.STAGE_NAME, 
                    "Triggering Jenkins Staging Deployment Pipeline(cd-staging)", 
                    releaseVersion,  
                    getLastCommitAuthor()
                ), '#00994c')

                //trigger cd-dev
                build job:"aem-projects/${L1_REPO_NAME}/cd-staging", parameters: [
                ...
                build job:"aem-projects/${L1_REPO_NAME}/hotfix/hotfix-staging", parameters: [
...
def semanticRelease(){
    ...
    if (oldReleaseVersion != releaseVersion) {
        ...
        println "Deployment aborted due to improper commit message. Please follow this commit message syntax to deploy it to Stage environment - fix(stage): XXX"
...
def pushL1HotfixTag() {
    ...
    def pkgJson = readJSON file: 'package.json'
    //releaseVersion = "hotfix_" + pkgJson.version + "_" + new Date().format('yyyyMMddHHmmss').toString()
    releaseVersion = env.GITHUB_BRANCH_NAME + "_" + pkgJson.version + "_" + new Date().format('yyyyMMddHHmmss').toString()
    // push hotfix tag to L1 Github
...
def getPipelineTriggeredBy(Boolean isWebhookTrigger, Boolean isHotfixBranch){
    ...
    if (isWebhookTrigger && !isHotfixBranch) {
        ...
        entrypoint = "main" //change from main to ci
    ...
    if (entrypoint == 'main'){
        ...
        if (JENKINS_PIPELINE_ID != 'staging') {
            ...
            if (releaseVersion ==~ releasePattern) {
                ...
                isDeployable = true
            // prod-release tags need to prepared for release before they can be deployed to a production environment
