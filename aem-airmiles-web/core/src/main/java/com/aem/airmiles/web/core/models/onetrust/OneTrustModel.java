...
public class OneTrustModel {
     ...
     */
    @OSGiService
    private @NotNull SystemResourceResolverService system;

    /**
     * The Sling settings service, used to determine the current run mode.
     */
    @OSGiService
    private @NotNull SlingSettingsService slingSettingsService;
    ...
    protected void init() {
        ...
        try (final ResourceResolver resolver = system.retrieveSystemUserResourceResolver()) {
            ...
            if (null == otCac || isAnyBlank(otCac.consentId(), otCac.sdkStubScript())) {
            ...
            log.info("Using CAConfig as token: '{}' and sdkStubScript:'{}'",
                    this.consentId, this.sdkStub);
    ...
    public Boolean isEnabled() {
        return !isAnyBlank(consentId, sdkStub);
    }
}
